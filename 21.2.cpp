...
int main(){
	...
	ifstream infile2("input21.txt");	while(getline(infile2,temp)){
		int posi=0;
		while(temp[posi+1]!='('){
			bool pricti=true;
	...
	cout << "Part 2: " <<allergens[2].possible[0]<<","
						<<allergens[3].possible[1]<<","
						<<allergens[6].possible[1]<<","
						<<allergens[1].possible[0]<<","
						<<allergens[0].possible[0]<<","
						<<allergens[4].possible[1]<<","
						<<allergens[7].possible[0]<<","
						<<allergens[5].possible[0]<<endl;
	return 0;
}